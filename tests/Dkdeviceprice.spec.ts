...
test('test',async ({browser }) =>{
  ...
  await page1.frameLocator('frame[name="leftFrame"]').locator('input[name="cprNumber"]').click();
  await page1.frameLocator('frame[name="leftFrame"]').locator('input[name="cprNumber"]').fill(testdata.taxpayer);
  await page1.frameLocator('frame[name="leftFrame"]').getByRole('link').nth(2).click();
  const locator= page1.frameLocator('frame[name="leftFrame"]').getByRole('cell', { name: 'Fornavn:* Efternavn:* E-mail:* Kontaktnummer:* C/O adresse CPR-adresse:* Postnummer:* By:* C/O adresse CPR-adresse:* Postnummer:* By:* Samtykke:* Version 2:Jeg giver samtykke til, at 3 må kontakte mig på e-mail, telefon, sms/mms og sociale medier for at blive holdt opdateret med nyheder, tilbud på produkter og tjenester samt konkurrencer fra 3. Kommunikationen kan vedrøre mit abonnement, hardware og tilhørende teleydelse samt tilbehør, som 3 tilbyder. Derudover giver jeg med mit samtykke lov til, at 3 må behandle mine forbrugs-, lokaliserings- og trafikdata til at give mig tilbud baseret på mit forbrug. Jeg kan til enhver tid tilbagekalde mit samtykke. Nej, jeg ønsker ikke at blive kontaktet af 3.' }).getByRole('img').nth(1);
